-- Retrieving data from the target table before performing the merge operation to observe existing records
...
-- Retrieving data from the target table after the merge operation to verify changes
SELECT * FROM TPCDS.INTERMEDIATE.CUSTOMER_SNAPSHOT;
...
-- Creating or replacing a table named CUSTOMER_DIM in the ANALYTICS schema, 
...
CREATE OR REPLACE TABLE TPCDS.ANALYTICS.CUSTOMER_DIM AS
    ...
    WHERE 
        END_DATE IS NULL -- Filtering the data to include only records where the END_DATE is null, indicating they represent the current version of customer information.
    );  
